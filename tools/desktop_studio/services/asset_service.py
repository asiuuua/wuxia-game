...
def _ensure_gdignore():
    ...
    p = os.path.join(MODULE_DIR, ".gdignore")
    if os.path.exists(p):
        return
    try:
...
# ============================ UI 界面贴图（.tscn 经 tscn_assets 适配器） ============================
...
#   * 新图不预生成 .import（交给 Godot 打开工程时自动导入，避免残缺 .import 导致 Godot 跳过导入而加载失败）。
