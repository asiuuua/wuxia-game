...
- **图标替换预留（2026-08-29）**：`scenes/ui/icon_registry.gd` + `UIManager.get_icon(id)/has_icon(id)` 为美术接入接口；资源集中在 `resources/icons/<分类>/<名>.<ext>`，按 id（不含扩展名、可带子目录，如 `skills/fire_sword`）引用；缺图标返回占位图绝不 null 崩；美术加图=丢文件、换图=同名覆盖（零代码）。规范 `docs/图标替换与美术接入规范.md`。⚠️ 新图标资源需 `--editor --quit` 生成 `.import` 才被门禁识别（`.import` 进库、`.godot/` 忽略；本工程门禁 `--quit` 不导入新资源）。
  ...
  - **已接线（UI 窗口主权，2026-08-29）**：背包 `ItemSlot`（`items/<item_id>`）、武学屏 `AbilitiesScreen`（`skills/<id>`）、结缘屏 `BondRomanceScreen`（`npc/<npc_id>`）；三处均零数据改动。战斗 `BattleScene` 图标（技能栏+敌人）已发派单 `f11a954808c2`（UI→战斗），待战斗窗认领。
