# 图标放在这里（美术 / 美工专用）

> 这个文件夹是**所有游戏图标的唯一来源**。你（美术）以后加图标、换图标，**只动这个文件夹**，不用碰任何代码。

## 怎么加一个新图标
1. 准备好图片（建议 `PNG`，透明背景；也支持 `webp` / `svg` / `jpg`）。
2. 按"分类"丢进对应子文件夹，文件名用英文、见名知意：
   ```
   resources/icons/
   ├─ items/        物品图标（丹药、武器、材料、任务品…）
   │   ├─ heal_pill.png         ← 文件名 = 物品 id
   │   └─ iron_sword.png
   ├─ skills/       武学 / 技能图标
   │   └─ fire_sword.png        ← 文件名 = 技能 id
   ├─ npc/          NPC 头像 / 立绘（对话立绘 + HUD 头像都放这里）
   │   ├─ su_waner.png          ← 对话立绘：文件名 = NPC 角色 id
   │   └─ player.png            ← HUD 玩家头像：固定叫 player.png
   ├─ enemies/      敌人头像 / 血条图标（战斗界面用）
   │   └─ boss_yan.png          ← 文件名 = 敌人 id
   ├─ status/       状态（增益/减益）图标
   │   └─ burn.png
   ├─ menu/         菜单按钮图标（主菜单 + ESC 菜单）
   │   ├─ save_game.png         ← 文件名 = 菜单项 key
   │   └─ settings.png
   ├─ ui/           其它界面装饰图标（地图、背包标记等零碎按钮）
   │   └─ map.png
   └─ _sample/      示例（sample_heart.png，可删）
   ```
3. 告诉策划 / 程序这个图标的"名字"，格式是 `分类/文件名`（**不带扩展名**），例如：
   - 技能图标：`skills/fire_sword`
   - NPC 对话立绘：`npc/su_waner`
   - 菜单按钮：`menu/save_game`
   - 玩家头像：`npc/player`
   
   程序会在对应配置里挂上这个名字，游戏自动显示。**绝大多数图标名字直接就是游戏里已经有的"物品 id / 技能 id / 角色 id / 菜单 key"，你不用新造名字**——按上面的对照表丢文件即可。

## 怎么"替换"一个旧图标
- 直接**用同名文件覆盖**即可（例如新的 `items/heal_pill.png` 盖掉旧的）。游戏里立刻生效，不用改任何配置。

## 各位置该丢哪个目录（速查表）
| 游戏里的位置 | 丢进目录 | 文件名怎么定 |
|---|---|---|
| 背包里的物品图标 | `items/` | = 物品 id（如 `heal_pill.png`） |
| 武学屏技能行图标 | `skills/` | = 技能 id（如 `fire_sword.png`） |
| 结缘屏 NPC 头像 | `npc/` | = NPC 角色 id（如 `su_waner.png`） |
| **对话里的立绘** | `npc/` | = NPC 角色 id（同上，复用一张图即可） |
| **HUD 左上玩家头像** | `npc/` | 固定叫 `player.png` |
| 战斗里敌人头像/血条 | `enemies/` | = 敌人 id（如 `boss_yan.png`） |
| 主菜单 / ESC 菜单按钮 | `menu/` | = 菜单项 key（如 `save_game.png`） |
| 其它零碎界面装饰 | `ui/` | 见名知意 |

> 记不住也没关系：**同一个人/物在对话和头像里共用 `npc/` 下同一张图**，不用画两张。

## 图片建议
- 尺寸：正方形，建议 **64×64 或 128×128**（2 的幂最好），放大缩小都清晰。
- 透明背景（PNG 带 alpha），边缘干净。
- 不要带中午文件名；名字用英文、下划线分隔。

## 找不到图标会怎样？
- 游戏不会崩。缺图标的地方会显示一个**品红色棋盘占位图**，提醒"这里还缺一张图"。
- 所以你随时可以只丢一部分图，剩下的先占位，不影响程序跑。

## 当前图标是"过渡占位"（请美术后续替换）
- 现在 `skills/`、`items/`、`enemies/`、`npc/`、`menu/`、`status/` 下已有的图标，是**程序自动生成的过渡占位图**（带分类配色的圆角汉字徽标，如「青松」「铁甲」）。
- 它们**不是最终美术资产**，只是用来替换原来的"品红棋盘占位"、让界面先能完整显示；质量达到"看得清、不崩、不紫块"即可。
- **替换方式不变**：美术按上面的"分类/文件名 = id"规则，用同名真实 PNG 覆盖即可，游戏立刻生效，无需改代码。
- **重新生成 / 批量补图标**：`tools/gen_icons.py`（纯 Pillow，无第三方美术依赖）会按数据配置里的 id 和中文名重新生成全部占位图标；已存在的文件不会被覆盖（保留美术真图）。
  ```bash
  python tools/gen_icons.py            # 生成全部缺失图标（已存在跳过）
  python tools/gen_icons.py --force    # 强制重生成（会覆盖程序生成的，但保留美术真图）
  ```

## 给程序看的（不用管，了解即可）
- 图标解析规则在 `scenes/ui/icon_registry.gd`，统一入口是 `UIManager.get_icon(<名字>)`。
- 详细规范见 `docs/图标替换与美术接入规范.md`。
