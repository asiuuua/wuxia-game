...
# P4 战斗实体对象池单测（按场景作用域实例）：验证 复用 / 重置清零 / 有界回收 / HUD 复用归零
# 不依赖场景树（BattleEntity/UnitHud 的 setup 已惰化为无树也可构建子节点）。
# 注意：CombatEntityPool 已改为实例化（每场战斗持有），测试用 pool.new() 模拟。

extends TestBase
class_name TestCombatEntityPool

var _pool: CombatEntityPool = CombatEntityPool.new()
...
func test_acquire_new_then_reuse() -> void:
	var e1 := _pool.acquire_entity("a", false, "敌A", 100, 50, null)
	expect(e1 != null, "acquire 应返回实体")
	expect(e1._built, "首次 acquire 应已构建视觉子节点")
	_pool.release_entity(e1)
	var e2 := _pool.acquire_entity("b", true, "玩家", 200, 80, null)
...
func test_reset_clears_pops_on_reuse() -> void:
	var e := _pool.acquire_entity("x", false, "敌", 100, 50, null)
	e.set_hp(30)
	e.pop_text("命中", Color(1, 0, 0))
	_pool.release_entity(e)
	expect(e._pop_layer.get_child_count() == 0, "release/reset 后飘字应立刻清空")
	var e2 := _pool.acquire_entity("y", false, "敌2", 100, 50, null)
	expect(e2 == e, "应复用同一实例")
...
func test_hud_pool_bounded() -> void:
	for i in range(0, 80):
		var h := _pool.acquire_hud("敌%d" % i, 100, 50)
		_pool.release_hud(h)
	expect(_pool.get_free_hud_count() <= 64, "HUD 空闲表应被裁剪到上限 64 以内")
