...
7. 工作室桌面工具（平台窗口主权）：唯一真源在 D:/武侠游戏/tools/desktop_studio/，只准改 index.html 与 studio_server.py，启动用 studio_launcher.bat（源码模式，读磁盘最新文件，改完刷新即生效）。严禁碰 dist/ 里的 .exe（那是冻结的旧构建，改源码它不会变，多个 AI 容易误以为"对接上了"其实连的是旧版）；严禁在别处复制一份改——那样会和我（平台窗口）的改动脱节、永远对接不上。要出发行版 exe 才用 PyInstaller 重打包（工作室专业调教.spec），不要手动维护 exe。
