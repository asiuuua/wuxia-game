# 14 Presentation / Input / ViewModel 施工图 V1.4 修复版

> 状态：FROZEN（2026-09-06 用户批准；可依此实施）
> 真源：宪法 §31/§32/§92/§97/§100（PROJECT_CONSTITUTION_V1.4.md L1680-1692 / L2097-2107 / L4080-4104 / L4194-4207 / L4240-4251）、01 图 §31 Layer 06 Presentation（L681-689）
...
> **V1.4 修复版总注**：本图随宪法 V1.4（ADR-0005）与 01 图 V1.4 修复版同步升版——①宪法条款号零漂移，正文「宪法 §N / 0-C.x / 0-F.x」引用全部有效；②RULE 001 放软：Domain 经白名单 Adapter/Boundary 触达 Godot 属合法协作（判据=0-E.3/GATE15，白名单升表随 ACR）；③本图冻结契约与冻结物版本零变化，V1.2 原稿保留（§171 收编不丢弃）。域内衔接：表现层装配自由与 0-B.0 分治双轨口径一致；UI 写行为扫描缺位仍为对齐审计在办偏差项（与本图升版互不影响）。
