...
- **Authority**：01 总体架构施工图 V1.2（`docs/architecture/01_总体架构施工图_V1.4修复版.md`（V1.2 稿为历史基线））
...
> **V1.4 修复版（对齐宪法 §76 统一句）**：`Result<T>` 仅为通用载体概念；业务层一律用上述具体强类型 Result，两说并存时以本条为准。
...
> **V1.4 修复版（RULE 001 放软口径，对齐宪法 0-B.0/0-E.3 与 01 图 §35 复校句）**：K-R01/K-R02 的 Godot 禁令针对**业务类内联调用**；代码明确属于白名单 Adapter/Boundary（`*_adapter.gd`、`adapter/` 层，命名/路径机器可验）时放行——Domain 核心业务状态与规则仍不得绑定 Godot 生命周期。判据落地与 GATE22 基线白名单升版随 ACR。
