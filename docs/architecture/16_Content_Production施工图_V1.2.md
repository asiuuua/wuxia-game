...
- 项目宪法 V1.2（L60 内容生产规范 / L308~320 Raw JSON 允许 / L1595~1611 数据化目标）
- 01_总体架构施工图_V1.2.md（§44 ID 规范 / §92 Validators / §128 Phase A 序列）
- 02_Domain_Kernel施工图_V1.2.md（ErrorCode / Result / ValidationViolation）
- 03_Contract_Schema_DataContract施工图_V1.2.md（§3 ID 契约 / §3.4 ADR-0002 / §4 Reference / C-R01）
- 05_Content_Registry_Content_Pipeline施工图_V1.2.md（CT/VA/PK/VE/DM 运行时机器；C-3 分片归属主权）
- 12_Quest_Dialogue_Story施工图_V1.2.md（任务/对话/Story Graph 结构契约）
- 13_Save_Persistence_Migration施工图_V1.2.md（存档红线与迁移链）
- 14_Presentation_Input_ViewModel施工图_V1.2.md（PV-8 本地化）
- 15_Studio_Authoring_Validator_Preview施工图_V1.2.md（ST-2 DataSink / ST-6 校验器注册表 / ST-4 studio 拆分）
