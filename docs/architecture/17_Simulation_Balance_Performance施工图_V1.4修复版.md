# 17_Simulation_Balance_Performance施工图_V1.4修复版
...
> 上游：项目宪法 V1.4（最高执行标准）→ 01 总体架构 → 06 Actor/Scheduler → 07 时间域 → 10 经济 → 11 战斗 → 04 测试基建 → 16 内容生产
...
> **V1.4 修复版总注**：本图随宪法 V1.4（ADR-0005）与 01 图 V1.4 修复版同步升版——①宪法条款号零漂移，正文「宪法 §N / 0-C.x / 0-F.x」引用全部有效；②RULE 001 放软：Domain 经白名单 Adapter/Boundary 触达 Godot 属合法协作（判据=0-E.3/GATE15，白名单升表随 ACR）；③本图冻结契约与冻结物版本零变化，V1.2 原稿保留（§171 收编不丢弃）。域内衔接：§93 性能下沉出口（V1.4 治理核心之一）与本图 Performance Budget 面衔接；GATE40 物理槽性能基准已入 verify_all --tier performance。
...
> **【已追认 2026-09-06】** 用户整批复核：以下 SBP-1~SBP-4 全部按推荐执行；其中 SBP-3/4 已按推荐实施（GATE40+ 三基准 + JSON 基线 + verify_all `--tier performance`）。本表保留原文供审计。
...
- 项目宪法 V1.4（§93 性能原则 / §94 大世界 NPC / §94A Performance Budget / §121 Simulation / §122 Headless / §216 Performance Regression / §230 第一阶段禁止项）
- 01_总体架构施工图_V1.4修复版.md（§92 Validators / §127 Gate 体系 / §128 Phase A 序列）
- 04_Test_Infrastructure_Architecture_Gate施工图_V1.4修复版.md（verify_all V2 --tier / golden 基线体系——Benchmark 门禁宿主）
- 06_Actor_Player_NPC施工图_V1.4修复版.md（Scheduler 五态 Tick / NPC 数据契约）
- 07_World_Time_Schedule施工图_V1.4修复版.md（AdvanceTimeCommand / TimeConsumer / 天气 randi 归属）
- 08_Relationship_Faction施工图_V1.4修复版.md（pregnancy 双时间源核查归属）
- 10_Economy_Shop_Crafting施工图_V1.4修复版.md（EC-7 团灭惩罚 / P-E7 死配置同族）
- 11_Ability_Combat_CombatAI施工图_V1.4修复版.md（AB-3 AIProfile / AB-4 数值真源 / AB-5 冷却 / AB-6 确定性）
- 15_Studio_Authoring_Validator_Preview施工图_V1.4修复版.md（ST-6 校验器注册表——新扫描器挂载点）
- 16_Content_Production施工图_V1.4修复版.md（CP-4 版本契约 / CP-5 校验器群 / P-CP4 version 违例）
