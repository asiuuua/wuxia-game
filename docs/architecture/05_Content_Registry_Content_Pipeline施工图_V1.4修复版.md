# 05 Content Registry / Content Pipeline 施工图 V1.4 修复版

| 项 | 值 |
|---|---|
| 状态 | **FROZEN**（2026-09-06 用户批准；可依此实施） |
| 日期 | 2026-09-05 |
| 上游 | 宪法 V1.4 → 01 §44~47（Content ID / Registry / Index / Binding）·§98（Content Schema 属 Shared Foundation）·§104（CONTRACT BEFORE IMPLEMENTATION）→ 03 §6（Content Pack Contract 已冻结布局与 manifest，本图不重复冻结）→ 04（Gate Registry / arch_lint / LN 编号） |
...
> **V1.4 修复版总注**：本图随宪法 V1.4（ADR-0005）与 01 图 V1.4 修复版同步升版——①宪法条款号零漂移，正文「宪法 §N / 0-C.x / 0-F.x」引用全部有效；②RULE 001 放软：Domain 经白名单 Adapter/Boundary 触达 Godot 属合法协作（判据=0-E.3/GATE15，白名单升表随 ACR）；③本图冻结契约与冻结物版本零变化，V1.2 原稿保留（§171 收编不丢弃）。域内衔接：GATE17 资产契约已挂物理槽（check_assets_contract.py --strict 进 verify_all 十四槽）；VE-1 指纹进 SaveHeader / 12 索引实建 / Mod 拒载等 DoD 3~6 项随批 2~3 落地，口径以本 V1.4 修复版为准。
...
- `PROJECT_CONSTITUTION_V1.4.md`（§44~47 内容架构 / §84 契约测试 / §88 Gate 体系 / §98 Shared Foundation / Provenance）
- `01_总体架构施工图_V1.4修复版.md`（§44 Content ID / §45 Registry 九职责 / §46 Index 12 张 / §47 Binding / §98）
- `03_Contract_Schema_DataContract施工图_V1.4修复版.md`（§3 ID / §4 Reference / §5 Version / §6 Content Pack / §7 序列化）
- `04_Test_Infrastructure_Architecture_Gate施工图_V1.4修复版.md`（§2 Gate Registry / §5 arch_lint / §6 Test Double）
- `02_Domain_Kernel施工图_V1.4修复版.md`（ValidationResult 家族 / DomainEvent COMMITTED 语义）
