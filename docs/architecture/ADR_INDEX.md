...
| ADR-0007 | **装配收敛实施案（18 Autoload → Composition Root 分批绞杀）** | 01 图 §73/§74/§128 + 宪法 0-B.0/0-G.8 依据下的 Phase3 实施案：目标形态=ApplicationRoot 唯一装配点（§73 冻结链），GameManager 瘦身为 Presentation 壳不删除；分批 A 骨架（零行为开关）→B 首批三件（shop/quest/inventory，含 ShopService Runtime 上提注入升表口）→C ConfigManager 收编→D 瘦身收口（Autoload ≤8 且全为引擎壳）；Write Lease 已落地（tools/write_lease.py，0-G.8 FROZEN_SCOPE 无锁并行禁区载体）；Phase D（C3）按 02 图 O-4 既定为各模块增量通道非集中批次；风险对策/回滚/DoD 详见 ADR 文档 | **APPROVED**（2026-09-06 用户批准；**批A 骨架同日落地**——application/application_root.gd Create/Inject 段+GameManager.USE_APPLICATION_ROOT 开关（默认 false）+_adopt_from 引用转发+test_application_root 4 用例，双开关各一次 77 套件全绿等价性验证；批B 首批三件待批A 观察期后实施） |
