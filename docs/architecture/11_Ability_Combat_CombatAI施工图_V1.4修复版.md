# 11 Ability / Combat / Combat AI 施工图 V1.4 修复版
...
> **V1.4 修复版总注**：本图随宪法 V1.4（ADR-0005）与 01 图 V1.4 修复版同步升版——①宪法条款号零漂移，正文「宪法 §N / 0-C.x / 0-F.x」引用全部有效；②RULE 001 放软：Domain 经白名单 Adapter/Boundary 触达 Godot 属合法协作（判据=0-E.3/GATE15，白名单升表随 ACR）；③本图冻结契约与冻结物版本零变化，V1.2 原稿保留（§171 收编不丢弃）。域内衔接：0-B.12 三层语义（Signal/DomainEvent/Command）为本图事件面口径；CombatantSnapshot 判据（ADR-0003）与确定性铁律不变。
...
- `01_总体架构施工图_V1.4修复版.md` §61 Ability / §62 Combat / §63 Combat AI / §101 AI 边界 / §111 VS-003 / §127 Gate 基线
- `02_Domain_Kernel施工图_V1.4修复版.md` Command·Query·Event / ErrorCode / Transaction / Result
- `03_Contract_Schema_DataContract施工图_V1.4修复版.md` ID 白名单 / 本地化 key / Definition 四态
- `04_Test_Infrastructure_Architecture_Gate施工图_V1.4修复版.md` Gate Registry / Test Double（FakeClock·SeededRandom）/ GATE26
- `05_Content_Registry_Content_Pipeline施工图_V1.4修复版.md` skills/status_effects/attribute_table/battles Pack 收编
- `06_Actor_Player_NPC施工图_V1.4修复版.md` Combatant Snapshot 数据源 / Tick 禁直改 / VS-005 物化
- `07_World_Time_Schedule施工图_V1.4修复版.md` GameClock / RNG Seed 进回放 / 禁全局 randi
- `09_Item_Inventory_Equipment施工图_V1.4修复版.md` InventoryTransaction / EntityId 分配器 / 来源留痕
