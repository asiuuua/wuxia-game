# 06 Actor / Player / NPC 施工图 V1.4 修复版

| 项 | 值 |
|---|---|
| 状态 | **FROZEN**（2026-09-06 用户批准；可依此实施） |
| 日期 | 2026-09-05 |
| 上游 | 宪法 V1.4 → 01 §34~38（Actor 架构 / Materialization / Player / NPC / State Ownership）·§54 Schedule·§66 Actor Scheduler·§113 VS-005 → 02（EntityId/Command 事务语义）→ 04（Gate Registry/双替）→ 05（ContentRegistry/Definition/ID 校验） |
...
> **V1.4 修复版总注**：本图随宪法 V1.4（ADR-0005）与 01 图 V1.4 修复版同步升版——①宪法条款号零漂移，正文「宪法 §N / 0-C.x / 0-F.x」引用全部有效；②RULE 001 放软：Domain 经白名单 Adapter/Boundary 触达 Godot 属合法协作（判据=0-E.3/GATE15，白名单升表随 ACR）；③本图冻结契约与冻结物版本零变化，V1.2 原稿保留（§171 收编不丢弃）。域内衔接：01 图 §36/§37 直修（Player 不设独立 Owner、NPCState 只读引用）与本图 State Owner 落位同口径；RULE 001 放软不改变本图边界冻结与 Owner 表。
...
> **【已追认 2026-09-06】** 用户整批复核：AC-1 按「B + 敌人补前缀」落定（并入 ADR-0002 扩展，登记见 `ADR_INDEX.md`）；AC-2~AC-4 按倾向执行。本表保留原文供审计。
...
- `PROJECT_CONSTITUTION_V1.4.md`（State Owner / Command 事务 / Deterministic Random / Test 体系 §81~87）
- `01_总体架构施工图_V1.4修复版.md`（§34~38 Actor/Player/NPC/Owner / §54 Schedule / §66 Actor Scheduler / §105 Combat Snapshot / §113 VS-005）
- `02_Domain_Kernel施工图_V1.4修复版.md`（EntityId / Command·DomainEvent / Transaction 契约）
- `04_Test_Infrastructure_Architecture_Gate施工图_V1.4修复版.md`（Gate Registry / transaction_test / 回放）
- `05_Content_Registry_Content_Pipeline施工图_V1.4修复版.md`（NPCDefinition 来源 / RegionPack / Index）
