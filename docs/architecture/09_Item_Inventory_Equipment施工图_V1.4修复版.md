# 09 Item / Inventory / Equipment 施工图 V1.4 修复版

| 项 | 值 |
|---|---|
| 状态 | **FROZEN**（2026-09-06 用户批准；可依此实施） |
| 日期 | 2026-09-05 |
| 上游 | 宪法 V1.4（0-C.19 Purchase Golden Invariant / 0-C.20 事务测试矩阵 / L2058-2065 Owner 表）→ 01 §56 Item·§57 Inventory·§58 Equipment·§60 Shop 边界 → 02（Transaction 契约/EntityId）→ 04（Gate/双替）→ 05（Definition/校验）→ 06（iid 分配器） |
...
> **V1.4 修复版总注**：本图随宪法 V1.4（ADR-0005）与 01 图 V1.4 修复版同步升版——①宪法条款号零漂移，正文「宪法 §N / 0-C.x / 0-F.x」引用全部有效；②RULE 001 放软：Domain 经白名单 Adapter/Boundary 触达 Godot 属合法协作（判据=0-E.3/GATE15，白名单升表随 ACR）；③本图冻结契约与冻结物版本零变化，V1.2 原稿保留（§171 收编不丢弃）。域内衔接：0-C.19 黄金不变式已由 D-10 收编实证（ShopTradeTransaction 的 TradeGoldenInvariant 进 commit Invariant 链，VS-001 永久 Golden）；TX-3 各写各 Owner 已物理化（MutationJournal 按 Owner 分账）。
...
> **【已追认 2026-09-06】** 用户整批复核：以下 IE-1~IE-4 全部按倾向执行（本表保留原文供审计）。
...
- `PROJECT_CONSTITUTION_V1.4.md`（0-C.19 Purchase Golden Invariant / 0-C.20 事务测试矩阵 / L2058-2065 Owner 表）
- `01_总体架构施工图_V1.4修复版.md`（§56 Item / §57 Inventory / §58 Equipment / §60 Shop 边界 / §108-109 VS-001）
- `02_Domain_Kernel施工图_V1.4修复版.md`（TransactionContext / MutationJournal / ErrorCode / EntityId）
- `06_Actor_Player_NPC施工图_V1.4修复版.md`（分配器 / AM-1 回环一致）
- `07_World_Time_Schedule施工图_V1.4修复版.md`（TX-4 禁自建序列同源 / TimeConsumer）
- `08_Relationship_Faction施工图_V1.4修复版.md`（一处事实多处引用同源思想 / RM-2 直连清零）
- `05_Content_Registry_Content_Pipeline施工图_V1.4修复版.md`（items/*.json 归 Definition / 五层校验语义层）
