...
- **Document ID**：ARCH-03
- **Version**：1.2（**V1.4 修复版**：2026-09-06，按《宪法 V1.4 修复版》（ADR-0005）与《01 总体架构施工图 V1.4 修复版》已决口径升版；领域内容与冻结条款主体不变，V1.2 原稿保留为历史基线）
...
> **V1.4 修复版总注（对齐宪法 V1.4）**：①ID/Schema 契约面与既有 C-R 条款全部沿用（宪法 0-C 章 20 条零变化已核验，ADR-0005）；②`I` 前缀口径——具体类型契约可用 `I`，泛型模板化仓库（`IRepository<T>`）仍禁止（宪法 §0-B.5 界线句）；③Adapter/Boundary 命名（`*_adapter.gd`/`adapter/` 层）作为白名单形态在 ID/Schema 域同样适用，登记机器可验白名单随 ACR。
...
- 上位：`docs/constitution/PROJECT_CONSTITUTION_V1.2.md`、`docs/architecture/01_总体架构施工图_V1.4修复版.md`（V1.2 稿为历史基线）
- 平级：`docs/architecture/02_Domain_Kernel施工图_V1.4修复版.md`、`docs/architecture/三张工程图_V1.2.md`（历史基线）、`docs/architecture/ACR-0001_采纳V1.2宪法与目标架构迁移.md`（历史事实保留）
