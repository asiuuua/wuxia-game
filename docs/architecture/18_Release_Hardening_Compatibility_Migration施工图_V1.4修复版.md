# 18 Release Hardening / Compatibility / Migration Verification 施工图 V1.4 修复版
...
> **V1.4 修复版总注**：本图随宪法 V1.4（ADR-0005）与 01 图 V1.4 修复版同步升版——①宪法条款号零漂移，正文「宪法 §N / 0-C.x / 0-F.x」引用全部有效；②RULE 001 放软：Domain 经白名单 Adapter/Boundary 触达 Godot 属合法协作（判据=0-E.3/GATE15，白名单升表随 ACR）；③本图冻结契约与冻结物版本零变化，V1.2 原稿保留（§171 收编不丢弃）。域内衔接：本图 FROZEN 锚定 V1.2 条款按冻结版执行，冲突处以宪法 V1.4 为准（ADR-0005 已登记口径）；RH-4 provenance 的 constitution_version 写现行宪法版本（V1.4）。
...
- 每次正式构建生成 `provenance.json`（随包根目录或嵌入 exe 同级）：八字段必填 `build_id`（构建时间+短 hash）/ `source_revision`（git HEAD）/ `game_version`（RH-1 真源）/ `constitution_version`（V1.4）/ `architecture_version` / `content_version`（05 图 content_fingerprint 同源）/ `schema_version` / `save_schema_version`（=SaveManager.SAVE_VERSION）。
...
> **【已追认 2026-09-06】** 用户整批复核：RH-1（版本 0.5.0 起步）/ RH-2（日志 5MB×3 轮转）/ RH-3（死常量退役）**已执行**；RH-4 按推荐（Phase4 最小可玩闭环，触摸/性能按真机实测另开任务卡）。本节保留原文供审计。
...
- 宪法：`docs/constitution/PROJECT_CONSTITUTION_V1.4.md` §32 / §130 / §211 / §212 / §214 / §216 / §230 / L5973（序列点名）
