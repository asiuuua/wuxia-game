...
- **ADR-0007**：APPROVED（2026-09-06 用户批准）。**批A 已落地**：application/application_root.gd（Create/Inject 段+装配完整性自检）+ GameManager.USE_APPLICATION_ROOT 开关（默认 false 旧路径）+ _adopt_from 引用转发（调用方/测试零改动）+ 4 用例；双开关各 77 套件全绿等价验证。批B 已落地（同日）：Register 段移交（9 服务 saveable）+ ShopTrade Runtime 工厂注入（升表口）+ USE_APPLICATION_ROOT 转正默认 true。批C（ConfigManager 收编）待批B 观察期。
