...
- `IconRegistry`（静态预加载）、`GameLogger`、`DisplayServer`（安全区 `get_display_safe_area`）
...
- 图标：`get_icon(icon_id)` / `has_icon(icon_id)`（经 IconRegistry，绝不返回 null）
...
- 依赖图审计（中期#2）：**`autoload/ui_manager.gd` preload → `scenes/ui/icon_registry.gd` 为唯一真实向上依赖违例**（全局管理器直依赖场景文件，应走 EventBus 解耦，非阻断 minor 耦合）

## 开放问题 / 待办（派单）
- `AI交接日志` 残留：Toast / 列表 refresh 未池化，频繁开关有 GC 抖动 → **UI 主权性能优化（未做）**
- 中期#2 违例 `ui_manager → icon_registry` 经 handoff 派 UI/架构窗口走 EventBus 解耦

## 架构备注
- UIManager 是「视图层中枢」，全工程最高频被依赖；其自身应只依赖更基础的 autoload/core，不反向依赖 scenes（当前 icon_registry 违例即此）。
