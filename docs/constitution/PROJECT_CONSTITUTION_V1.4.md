AI驱动大型RPG工程施工总规范 V1.4 — Context Integrity Edition
Project Engineering Constitution
AI-Driven Large-Scale RPG Engineering Specification
Version: 1.4
Status: Foundation / Mandatory / Integrated Edition / Supersedes V1.3
V1.3 Baseline: Preserved as historical engineering baseline
Applies To: Runtime / Content / Tools / Studio / Tests / AI Collaboration / Build / Save / DLC / Mod

---
0.1 V1.4 修订说明
V1.4 是「Godot-Native Runtime + Enterprise-Grade Governance」治理落版。V1.4 不推翻既有治理骨架，而是把「Domain 不认识 Godot」这一绝对式核心铁律，放软并补足「经明确白名单 Adapter/Boundary 与 Godot 原生运行时深度协作」的合法出口；同时把 Signal / Domain Event / Command 三层语义显式化，并在 0-B 系列新增「双引擎定位」总领小节。

V1.4 继承并保留 V1.3：
- 全部 Context Integrity / Coverage / Recovery / Semantic Drift Governance 能力。
- Domain / Application / Infrastructure / Presentation 分层、模块主权、唯一状态 Owner、公开契约。
- AI Permission、Write Lease、ACR / ADR、Context Pack、Freeze Manifest、Compatibility、Provenance。
- GATE01~32、ESG（Enforcement / 扫描器）、门禁基线体系。
- 三张工程图（Architecture / Foundation / Build Graph）与 Vertical Slice 主线。

V1.4 新增或修订的关键定位（本次治理核心）：
1. RULE 001 从「Domain 不认识 Godot（绝对禁用）」放软为「核心业务状态与规则不得绑定 Godot 生命周期 / 不得直接内联 Godot Engine API；允许经明确的白名单 Adapter/Boundary 与 Godot 原生运行时深度协作」。
2. ADR-0001 修订：结论细化为「核心业务不绑定 Godot 生命周期 + 允许经 Adapter 协作」，并注明适用边界与反例（如 Domain 内直接 `get_tree()` 为反例）。
3. 新增 0-B.0 定位小节「分治双轨」：治理（架构 / Contract / Owner / 权限 / 版本 / Gate / Migration / Change Management）——企业化；运行时（Scene / Node / Resource / Signal / Animation / Input / Physics / Navigation / SceneTree / Rendering / Audio / 生命周期）——Godot 原生；交界处仅通过 Adapter / Boundary / Port / Presentation / Runtime Shell 连通。
4. 0-B.12 Signal Policy 增补三层语义显式化：Godot Signal（本地运行时 / UI / 生命周期）/ Domain Event（跨业务事实）/ Command（行为意图），三者命名与通道不得混用。
5. 0-E.3 Forbidden Dependency Check 保持「Domain → Godot 默认拦截」，但明确「白名单 Adapter（命名/路径，如 *_adapter.gd、adapter/ 层）」放行，可被机器验证。
6. GATE15 Domain Godot Access 判据就地批注：经白名单 Adapter 的协作属合法，不违反 GATE15。
7. §93 性能原则加注：性能所需的 Godot 下沉，走 Adapter 白名单后可到达 Runtime Shell / Rendering / Server。

V1.4 治理来源：《2026-09-06 宪法V1.3按对话治理_建议清单》（A-1~A-7）。本次治理依据用户授权触及核心铁律级条款，原 V1.3 副本完整保留、不覆盖。

---

0.2 V1.3 修订说明
V1.3 不推翻 V1.2 的治理骨架，而是在 V1.2 之上完成一次面向长上下文、上下文压缩、模型切换与多 AI 交接的工程化完整性增强。
...
V1.3 新增或强化的核心工程能力：
1. Context Integrity：区分 Context Pack 文件完整性与 AI 当前语义完整性。
2. Context Coverage：为每个 Task 声明并验证必需知识集合，防止“包有效但知识不全”。
3. Context Recovery：上下文压缩、Handoff、Model Switch、Session Resume 后必须恢复并重验。
4. Compression Event / Semantic Context Drift：把模型内部上下文变化正式纳入治理。
5. Knowledge Source Class：明确 AUTHORITATIVE / DERIVED / HISTORICAL / PROPOSAL / UNKNOWN。
6. AI_CONTEXT.md 降级为 DERIVED 辅助记忆，不得成为权威事实源。
7. GATE30 从 Context Freshness 升级为 Context Integrity & Freshness。
8. Context Recovery 失败、覆盖不足或语义冲突时，统一进入 STOP / BLOCKED / ACR_REQUIRED。

V1.2 基线新增或强化的核心工程能力（保留）：
...
V1.3 = V1.2 + Context Integrity / Coverage / Recovery + Semantic Drift Governance
V1.4 = V1.3 + Godot-Native Runtime + Enterprise-Grade Governance（RULE 001 放软 + Adapter 出口 + Signal 三层显式化 + 0-B.0 双引擎定位）

V1.4 生效后：
V1.3 保留为历史基线；冲突时，以当前已批准且版本号最高的 V1.4 Project Constitution 为准。
V1.2 中未被 V1.3 明确修改的条款继续有效。
...
RULE-GODOT-DOMAIN
Rule：Domain 不得依赖 Godot（核心业务不绑定 Godot 生命周期；经白名单 Adapter / Boundary 的深度协作属合法，见宪法 V1.4 RULE 001 / GATE15 / 0-E.3）
Severity：FATAL
Enforcement：E3 / E4
...
---
0-B.0 双引擎定位（Godot-Native Runtime + Enterprise-Grade Governance）

本规范采用分治双轨，作为所有 Godot 相关规则的总领原则：

- **治理侧——企业化**：架构 / Contract / State Owner / 权限 / 版本 / Gate / Migration / Change Management。这些是工程纪律，必须用企业级治理约束 AI 与工具，不因其运行于 Godot 而弱化。
- **运行时侧——Godot 原生**：Scene / Node / Resource / Signal / Animation / Input / Physics / Navigation / SceneTree / Rendering / Audio / 节点生命周期。这些是 Godot 的原生机制，直接用其构建游戏，不为“模拟企业语言”而再造一套自绘运行时。
- **交界处**：只通过 Adapter / Boundary / Port / Presentation / Runtime Shell 连通。Domain 核心业务状态与规则不绑定 Godot 生命周期，也不直接内联 Godot Engine API；需要与 Godot 原生运行时深度协作时，一律走白名单 Adapter（详见 RULE 001、0-B.9、GATE15）。

作用：给每一层的后续规则一个统一思想锚点，避免“为了防止耦合而在 Domain 里复制一套 Godot 抽象”（正违背 0-A.7 最小实现原则）。
...
> **界线句（与 §27 对齐）**：允许的是**具体类型仓库接口**（如 `INPCRepository`、`IQuestRepository`，为具体实体服务的接口）；禁止的是**泛型模板化仓库**（如 `IRepository<T>`）。前者可用 I 前缀，后者不得作为架构前提（见 0-B.6）。
...
三种机制的语义分层必须显式区分，命名与 deliver 通道不得混用：

- **① Godot Signal**：Node 生命周期 / UI / Animation / 本地运行时通知（Presentation / local / adapter），为低耦合解耦用。
- **② Domain Event**：Gameplay 业务事实，跨模块事实传播（如 TimeAdvancedEvent / ContentReadyEvent）。
- **③ Command**：Gameplay 行为意图（intention），经 Command Scheduler / Dispatcher 投递，最终映射到 Transaction。
...
命名口径与已审施工图（02 / 04 / 09 的事件契约）对齐，Signal 不得伪装成 Domain Event 或 Command，反之亦然。
...
> **Mutation Journal 与 Event Log 边界（消除 B7 混淆）**：Mutation Journal（本 §0-C.5）记录**可逆写入的 before/after 与回滚原语**，属事务/回滚底层的单次 Mutation 级日志；Event Log（§119）记录**已发生的业务事实（事件序列）**，供回放/审计/派生状态。二者不混用：Journal 只在事务内、可被回滚撤销、随事务生命周期管理；Event Log 为跨模块事实流、保存策略与 Owner 按 §119 与 13 号图定，不在事务回滚范围内。
...
扫描规则：默认仍拦截 `Domain → Godot`（断言 Domain 核心业务不相依赖 Godot Engine API）。
白名单豁免：明确属于 **Adapter / Boundary** 的目录或命名（如 `adapter/` 目录、`*_adapter.gd`）放行——即 RULE 001 允许的「与 Godot 原生运行时深度协作」出口。豁免必须满足：
- 只在 Adapter / Boundary 文件内。
- 按命名 / 路径白名单逐项登记，可被 forbidden_api_validator 机器校验。
- 不反向成为“Domain 直接 import Godot 类”的后门（参见 0-E.4 Forbidden API Check 的白名单登记）。
...
13. Build Smoke
14. GATE30 Context Integrity & Freshness（Context Pack / Coverage / Manifest Hash 校验，进入 verify_all，见 §220 / §233 / §235）
...
---
0-G.11 Context Integrity

Context Pack 的版本完整性与 AI 当前推理上下文完整性属于两个不同维度。

Context Pack Integrity：
验证 Context Pack 本身是否来自有效的 Project Knowledge、Revision 与 Manifest。

Semantic Context Integrity：
验证 AI 在 Context Compression、Handoff、Model Switch、Session Resume、Context Rebuild 后，是否仍基于同一组权威工程事实施工。

以下任一事件发生后，必须触发 Context Recovery：
- Context Compression
- Context Window Rebuild
- Model Switch
- AI Handoff
- Session Resume
- Context Pack Regeneration
- Source Revision Change
- Relevant Contract Change
- Relevant ADR Accepted
- Context Coverage Change

Recovery 完成前：
AI 不得继续 IMPLEMENTATION、MERGE 或 Foundation 修改。
...
---
0-G.12 Context Coverage

Context Pack 不仅必须“有效”，还必须覆盖当前 Task 所需的最小知识集合。
...
- Relevant ADR / ACR
- Required Tests
- Current Freeze Manifest（适用时）
- 相关 Compatibility / Migration Policy（适用时）

Context Coverage Manifest 必须记录：
- task_id
- required_documents
- loaded_documents
- missing_documents
...
---
0-G.13 Context Recovery Protocol

发生 Context Recovery 时，AI 必须重新加载：
1. Current Constitution
2. Current Architecture
3. Current Module Map
4. Current Module Spec
5. Current Task
6. Relevant Contracts
7. Required Tests
8. Relevant ADR / ACR
9. Current Freeze Manifest（适用时）
10. Context Coverage Manifest
...
Recovery 后不得依据“旧的模型记忆”自行补回缺失事实。

---
0-G.14 Semantic Context Drift
...
非权威派生知识不得覆盖 AUTHORITATIVE Project Knowledge。
...
上下文哈希可以证明：
“Context Pack 文件没有被修改”。

不能单独证明：
“AI 在压缩后仍准确理解 Context”。

因此 Compression Event、Handoff、Model Switch、Session Resume 必须与 Context Integrity 一起治理。
...
---
2. 最重要的十条铁律
RULE 001：Domain 核心业务不绑定 Godot，允许经白名单 Adapter 协作
核心业务状态与规则不得绑定 Godot 生命周期，也不得直接内联 Godot Engine API。
Domain 默认禁止依赖：
Node
Node2D
Control
...
DirAccess
ProjectSettings
上述禁止项在以下情形放行：代码明确属于 Godot Adapter / Boundary（参见 0-B.9 Node Boundary、0-B.0 双引擎定位、0-E.3 白名单豁免）。
即：Domain 的核心业务状态与规则不绑定 Godot；但当业务需要与 Godot 原生运行时（Scene / Node / CharacterBody2D / Audio / Server 等）深度协作时，通过明确的白名单 Adapter / Boundary 连接，而不得在业务类内直接调用 Godot Engine API。
反例：在 Domain / Service 内部直接 `get_tree()`、`new CharacterBody2D()`、`Camera`、`FileAccess` 等属违规；正例：PlayerDomain 经 PlayerController Adapter 操作 CharacterBody2D。
Domain 只能处理：
数据
规则
状态
...
> **架构图底层名词统一（消除 B1 混淆）**：图中「Domain Contracts」即 **Kernel**，二者为同一层，落在 **`domain/kernel/`**（Kernel 是 Domain 的下属子层，不是平级独立层）。本图及全文两处图统一以此表述：最底层挂 **Kernel（= domain/kernel/ 契约集）**。Domain 与 Kernel 为「包含关系」——Domain 内含 Kernel，而非上下平级。
推荐目录：
project/
│
├── app/
...
> **Kernel 写 Owner 说明（消除 B2 混淆）**：Kernel 属**共享基础设施**，其内部契约（ID / Clock / Random / Repository Contract / Command-Query-Event 抽象等）由 infra / composition 统一装配，其契约由 02 施工图冻结；Kernel 内部**不设业务写 Owner**，为共享只读契约层（见 RULE 009 冻结治理）。
...
> **Foundation/Application/术语消歧（消除 B10/B11/B12，统一口径）**：
> - **「Foundation」一词三种含义**：① §0-A.3 **Foundation Graph** = 横向能力铺设链，改称 **Foundation Chain**；② §4 **Level 1 Foundation** = 模块稳定等级；③ **Shared Foundation**（RULE 009）= 需冻结单 Owner 治理的共享层。三义在文内按上下文取义，本句为准。
> - **Application** 既是横向基础层（§4 Level 1）之一，也是可列举模块；其同样适用 §168 模块 Owner 制度，**装配 Owner = Composition/ApplicationRoot**（见 RULE 009、三张图 A5）。
> - **术语对照**：`Domain Contracts ≡ 模块契约 ≡ domain/kernel/ 契约集`，Kernel 为 Domain 的下属子层（见 B1 统一句）；正文出现任一写法视为同义。
...
├── content/
└── tests/
> **模块 content/ 与顶层 content/ 分工（消除 B4 混淆）**：模块级 `content/` 为**模块内私有定义草稿**（研发期内容、随模块走）；所有**对外可见的 Definition / Schema / Pack / Index 统一上收到工程顶层 `content/`**（`content/schemas`、`definitions`、`packs`…）校验发布。判定规则：只被本模块消费 → 可留模块内；会被其他模块 / 工具 / 运行时公共索引消费 → 必须上收顶层并走 Content Pipeline（§64/§64A）。
模块施工图必须包含：
1. 模块定位
2. 职责
3. 非职责
...
> **AI 十问权威声明（消除 §10 vs §0-D.2 十问张力）**：施工前权威十问以 **§0-D.2 的「开工前十问」为权威模板**；本 §10 为原则层面的相同意图摘要。两者问答内容按 §0-D.2 执行，§233「AI 十问重验」亦以此为准。
...
> **AI 角色权威声明（消除 §0-G.2 vs §21 两表张力）**：全项目 AI 角色以 **§0-G.2 的 10 角色表为权威主表**（含 AI-EXECUTION / AI-INFRA / AI-ACTOR / AI-REVIEW）；本 §21 为**摘要/兼容表**，两者冲突时以 §0-G.2 为准。
...
例如：
ADR-0001
为什么 Domain 核心业务不绑定 Godot、但允许经 Adapter 协作

结论（V1.4 修订细化）：
Domain 的核心业务状态与规则不得绑定 Godot 生命周期，也不得直接内联 Godot Engine API；但当业务需要与 Godot 原生运行时深度协作时，允许经明确的白名单 Adapter / Boundary 连接。

适用边界：
- 正例：PlayerDomain ↦ PlayerController（Adapter）↦ CharacterBody2D；SoundDomain 经 Adapter 到达 AudioServer；性能下沉经 Adapter 到达 RenderingServer / Server。
- 反例：在 Domain / Service 业务类中直接调用 `get_tree()`、`new Node2D()`、`Camera`、`FileAccess`、`ProjectSettings` 等 Godot Engine API。

为什么不能“完全依赖 Godot”：绑定 Godot 生命周期会把业务状态、规则与渲染/节点树耦合，破坏可测试、可迁移、可多上下文复核的 Domain 纯度。
为什么不能“彻底禁止协作”：避免在 Domain 内复制一套 Godot 抽象（违背 0-A.7），当业务确需 Godot 原生能力时，白名单 Adapter 是唯一合法出口。

ADR 生效后：后续 AI 必须服从本结论；除非创建新 ADR 推翻旧 ADR（完整闭环见 §23A）。
...
> **NPC/Combat 状态同步 Owner 声明（消除 B9 混淆）**：同一 NPC 的状态同步明确由 **Progression（写 Level/Attributes 真源）→ Composer（装配层）** 组装为只读 **CombatantSnapshot** 供 Combat 读取；Actor Runtime 仅承载运行时载体，不复制真源。Progression 是等级/属性写 Owner；快照装配由 Composition/Composer 负责，任何模块不得越过 Composer 直接灌快照。
...
> **Story 归属声明（消除 B5 混淆）**：Story 归 **`domain/story/`**，属**领域编排队**，可承接横向 Application orchestration 意图，但**不新增独立模块等级、不替换 Quest/其他模块**（只组合、不重定义）。其编排经 Command/Effect/Event 触发 Quest/Dialogue，不在 Domain 内直接写其他模块状态。
...
> **Content 管线唯一权威声明（消除 B6 混淆）**：§64（Content Pipeline）、§64A（Content Review/Publish Gate）、§225（V1.3 Content 施工流程）为**同一管线的三层描述**；以 **§64A 为权威主流程**（含 Review/Publish Gate），§64、§225 为更上层摘要或阶段细化，冲突时以 §64A 为准。
...
> **Result 形态统一句（消除 §76 泛型 vs §0-B 具体类型张力）**：`Result<T>` 为**通用载体接口**；业务层与模块对外契约改用 §0-B.1/§0-B.16 的**具体强类型 Result**（如 `OperationResult`、`CommandResult`、`TransactionResult`）。§0-B.6 禁止的是“以泛型模板化仓库为架构前提”，不禁止 `Result<T>` 这类通用载体。
...
> **时钟/随机装配声明（消除 B8 混淆）**：IGameClock / IRandomProvider 由 **Composition Root（app/composition）统一实例化并注入**各模块（构造注入 / Port），不作全局单例搜索；对集成式 TimeConsumer（§234 / 07 图）与 SeededRNG（11 图）同样由 Composition 装配，测试注入替身走同一注入点。
...
（判据：Domain 默认不得直接访问/内联 Godot Engine API；经白名单 Adapter / Boundary 的协作属合法，不违反 GATE15。一致性见 ADR-0001 / RULE 001 / 0-B.0 / 0-E.3 白名单豁免）
GATE16 Studio Smoke
...
> **Gate 主清单一致性说明（消除多表取错）**：全项目 Gate 以本规范为单一真源，总表 = **§88（GATE01~20）∪ §228（GATE21~32）**。任何他处出现的 Gate 清单（§0-E.9 verify_all 步骤、04 施工图物理槽位、CSV 清单等）一律为派生视图，若与其冲突，以本规范总表为准；核查 Gate 时统一从本总表取。GATE15/GATE25/GATE30 编号与此主清单严格一致。
...
（性能所需的 Godot 下沉，走 Adapter 白名单后可达 Runtime Shell / Rendering / Server；不得因性能在 Domain 内再造 Godot 抽象，也不得把聚合逻辑硬留在 Domain 而拒绝应有的 Native 下沉）
...
V1.3 增强：Context Pack 必须额外记录：
source_class
coverage_manifest_hash
recovery_state
compression_event_id（如适用）
parent_context_version（如适用）
semantic_validation_result

---
133A. Context Knowledge Source Class

所有进入 AI Context 的知识必须标记：

AUTHORITATIVE
正式权威事实。来源包括：Current Constitution、Current Architecture、Frozen Contract、Accepted ADR、有效 Freeze Manifest、批准的 Module Spec。

DERIVED
从权威事实推导的辅助摘要。可以帮助导航，但不得改变权威事实。

HISTORICAL
旧版本、Legacy、SUPERSEDED 文档。只可用于历史解释、迁移、考古和 Bug 调查。
...
优先级仅用于识别信息来源，不授权 AI 自动择优。
同一主题出现冲突时：STOP。

---
133B. Context Coverage Manifest

每个动态 Context Pack 必须生成：
...
CONTEXT_INCOMPLETE 不允许进入 IMPLEMENTATION。

---
133C. Compression Event

上下文压缩属于正式工程事件。

每次可观测的 Context Compression / Rebuild 应记录：
compression_event_id
pre_context_version
post_context_version（如重新生成）
source_revision
trigger
recovery_required
recovery_result
...
压缩事件本身不得改变 Project Knowledge。

---
133D. Semantic Recovery

Context Recovery 后，必须重新验证：
1. 当前任务。
2. Module 与 State Owner。
3. Public Contract。
4. Transaction Boundary（适用时）。
5. Forbidden Dependency。
6. Required Tests。
7. Permission / Write Lease。
8. Relevant ADR。
...
AI_CONTEXT.md 属于 DERIVED KNOWLEDGE。
只能作为导航、常见坑与恢复辅助，不属于权威事实源。
...
与 Constitution / Architecture / Contract / ADR / Freeze Manifest 冲突时：
AI_CONTEXT.md 自动失效，必须回到权威来源重新生成。
...
> **总纲句（消除三套并存歧义）**：本规范全文的施工组织共三套，各自含义不同、不互相矛盾：
> 1. **文档编号路径（01~18）** = 文档目录 / 索引顺序，不代表依赖顺序。
> 2. **横向 Foundation Track** = 依赖铺序（能力如何被逐步建立）。
> 3. **阶段 A-D（§229 阶段 A-D 开工顺序）** = 实际开工执行序，是施工时唯一裁决依据。
> 三者在执行时统一以「阶段 A-D」为准；文档编号与横向轨用于理解依赖，不替代开工序。
...
V1.3 Context Integrity Propagation：
任何会影响 AI 当前工程认知的变化还必须传播到：
Context Coverage Manifest
Context Integrity
Context Recovery State
AI_CONTEXT.md（如存在）

发生 Context Compression / Model Switch / Handoff / Session Resume 时，
必须能够追溯到 Context Recovery Event ID。
...
Context Recovery 必须可追踪到 Recovery / Compression Event ID。
...
context_manifest_hash
context_coverage_manifest_hash
semantic_validation_result
gate_results
frozen_at
...
□ Context Coverage Manifest 已建立
□ Context Compression Event 可记录
□ Context Recovery Protocol 已建立并可执行
□ Semantic Context Drift 可以进入 Gate
□ Knowledge Source Class 已定义并可检查
□ AI_CONTEXT.md 已明确为 DERIVED KNOWLEDGE
□ GATE30 Context Integrity & Freshness 已进入 verify_all
...
---
221. V1.3 Context Integrity 整合版实施索引
...
> **Player 定位声明（消除 B3 混淆）**：Player 是 **Actor 的特殊实例**（承载玩家 Runtime Identity / Controller / 输入 Intent），不设独立 player 模块。Player 的可写领域状态一律按状态语义归各 Owner——例如 **Gold 归 Economy 的 Wallet**、等级归 Progression、背包归 Inventory，Player 自身不拥有这些领域的写所有权；Player 的控制器（Controller）在 presentation / adapter 层。

---
225. V1.3 Content 施工补充规则
...
> **措辞对齐（消除 §228 建议语气 vs §220/§233 硬验收的张力）**：GATE21~GATE32 一经引入即列为**硬性验收项**（非“可选的允许项”），其中 **GATE30 Context Integrity & Freshness 为 V1.3/V1.4 的强制必过 Gate**，必须进入 verify_all（见 §220/§233/§235 与 §0-E.9 第 14 步）。
...
让每一次 AI 修改都拥有范围、权限、版本、证据与 Context Integrity；
让每一次架构变化与 Context 变化都能够传播到 Contract、Context、Test、Gate 与代码；
让项目尽早通过真实 Vertical Slice；
让 AI 可以高速施工，但不能靠猜测、越权、隐式副作用和临时架构换取速度。

---
233. V1.3 Context Integrity 验收闭环

任何 AI Task 进入 IMPLEMENTATION 前，必须满足：
...
CONTEXT_RECOVERY_REQUIRED
 ↓
Reload Authoritative Knowledge
 ↓
Coverage Check
 ↓
Version / Revision / Hash Check
 ↓
Semantic Validation
 ↓
AI 十问重验
...
有效证据仍然只能来自：
Compiler / Static Check / Test / Gate / Reproduction / Benchmark / Build Artifact / Change Record / Authoritative Project Knowledge。

---
234. V1.3 Context 状态机

VALID
 ↓
COMPRESSION / HANDOFF / MODEL SWITCH / RESUME
 ↓
RECOVERY_REQUIRED
 ↓
COVERAGE_CHECK
...
V1.3 不允许 UNKNOWN 自动升级为 VALID。

---
235. V1.3 Context Integrity 最小机械化要求

第一阶段至少实现：
1. Context Pack Generator
2. Context Coverage Manifest Generator
3. Context Manifest Hash
4. Source Revision Check
5. Freeze Manifest Check
6. Relevant Contract Resolution
7. Relevant ADR Resolution
8. Context Freshness Gate
9. Context Recovery Marker
10. GATE30 Context Integrity & Freshness

第二阶段可以增加：
Semantic Validation Checklist
自动 Task-to-Knowledge 映射
Compression Event Telemetry
Recovery Replay
Context Coverage Trend

但在未实现这些自动化能力前，不得声称相关 Gate 已机械化。

---
236. V1.3 术语新增

Context Integrity
= Context Pack 的来源、版本、Revision、Manifest、Coverage 与 AI 当前施工依据的一致性。

Context Coverage
= 当前 Task 所需权威知识是否全部进入有效 Context 的可验证程度。

Context Recovery
= Context Compression、Handoff、Model Switch、Session Resume 后重新建立并验证施工上下文的过程。

Semantic Context Drift
= Context Pack 未必发生文件变化，但 AI 当前用于决策的理解已经偏离权威工程事实。

Compression Event
= 可观测的上下文压缩、重建或切换事件。

Knowledge Source Class
= 标记 Context 中信息是 AUTHORITATIVE、DERIVED、HISTORICAL、PROPOSAL 或 UNKNOWN。

---
237. V1.3 与 V1.2 的兼容原则

V1.2 中未被 V1.3 明确废止的条款继续有效。
V1.2 历史文件不得被删除。
V1.3 只增加 Context Integrity 治理，不改变已有 Domain / Application / Infrastructure / Presentation 分层、模块主权、Transaction、Save、Content、Vertical Slice 与多 AI Build Protocol 的基本语义。

当 V1.2 与 V1.3 在 Context 治理上存在冲突：
V1.3 优先。
...
READ BEFORE WRITE
UNDERSTAND BEFORE MODIFY
AUTHORITATIVE BEFORE DERIVED
COVERAGE BEFORE IMPLEMENTATION
INTEGRITY BEFORE CONTINUE
CONTRACT BEFORE IMPLEMENTATION
SCOPE BEFORE EDIT
OWNER BEFORE MUTATION
TRANSACTION BEFORE MULTI-STATE CHANGE
...
让每一次 AI 修改都拥有范围、权限、版本、证据与 Context Integrity；
让每一次架构变化与 Context 变化都能够传播到 Contract、Context、Test、Gate 与代码；
让上下文可以被压缩，但工程事实不能因为压缩而失真；
让 AI 可以高速施工，但不能靠猜测、越权、语义漂移、隐式副作用和临时架构换取速度。
