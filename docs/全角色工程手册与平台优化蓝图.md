...
4. **经验库增强检索引词**：当前 `docs/经验库/` 用文件名+关键词；应加标签云 + 按 BUG 号 / 模块 / 角色过滤，新 AI 问「点不了」直接命中 mouse_filter 铁律。**✅ 已落地（2026-09-02，commit `e718133` [工作室]）：`/api/experience` 服务端实时富化 `knowledge.refs`（解析「检索关键词」生成标签云、按角色/模块/BUG 分面），前端加标签云+分面过滤+变更通告默认隐藏开关；零 schema / 零文档改动。**
